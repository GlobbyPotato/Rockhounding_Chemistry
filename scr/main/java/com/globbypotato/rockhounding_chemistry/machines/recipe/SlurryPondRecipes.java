...
public class SlurryPondRecipes extends BaseRecipes{
	...
	public static void machineRecipes() {
		slurry_pond_recipes.add(new SlurryPondRecipe("compoundCoal", 											CoreBasics.waterStack(1000), 					getFluid(EnumFluid.COAL_SLURRY, 1000)));
		slurry_pond_recipes.add(new SlurryPondRecipe(new ItemStack(Items.ROTTEN_FLESH), 						CoreBasics.waterStack(200), 					getFluid(EnumFluid.ORGANIC_SLURRY, 200)));
		...
		if(!ModUtils.forestry_compost().isEmpty()){
			slurry_pond_recipes.add(new SlurryPondRecipe(ModUtils.forestry_compost(), 							CoreBasics.waterStack(200), 					getFluid(EnumFluid.ORGANIC_SLURRY, 200)));
		}

		for(int x = 0; x < EnumMinerals.size(); x++){
			slurry_pond_recipes.add(new SlurryPondRecipe(mineral_ores(1, EnumMinerals.values()[x]), 			getFluid(EnumFluid.SULFURIC_ACID, 250), 		getFluid(EnumFluid.LEACHATE, 250)));
		}
