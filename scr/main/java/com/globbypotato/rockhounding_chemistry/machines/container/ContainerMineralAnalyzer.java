...
public class ContainerMineralAnalyzer extends ContainerBase<TileEntityMineralAnalyzer>{
	...
	protected void addOwnSlots() {
		...
		this.addSlotToContainer(new SlotItemHandler(input, 0, 44, 30));//input
		this.addSlotToContainer(new SlotItemHandler(input, 1, 8, 8));//fuel
		this.addSlotToContainer(new SlotItemHandler(input, 2, 44, 60));//consumable
		...
		this.addSlotToContainer(new SlotItemHandler(output, 0, 76, 60));//output
		
		this.addSlotToContainer(new SlotItemHandler(template, 0, 96,  88));//drain
	...
	public ItemStack slotClick(int slot, int dragType, ClickType clickTypeIn, EntityPlayer player){
		if(slot == 7){
			...
			if(this.tile.gravity >= 1.00F){
				this.tile.gravity -= 0.50F;
			}
			doClickSound(player, tile.getWorld(), tile.getPos());
    		return null;
		}else if(slot == 9){
			if(this.tile.gravity <= 29.00F){
				this.tile.gravity += 0.50F;
			}
			doClickSound(player, tile.getWorld(), tile.getPos());
    		return null;
