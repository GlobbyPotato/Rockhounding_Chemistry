            ...
            if (!worldIn.isBlockModifiable(playerIn, blockpos)){
                ...
                if (!playerIn.canPlayerEdit(blockpos.offset(raytraceresult.sideHit), raytraceresult.sideHit, itemStackIn)){
                    ...
                    if (material == Material.WATER && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0){
                        ...
                        return new ActionResult(EnumActionResult.SUCCESS, this.fillBucket(itemStackIn, playerIn, ModFluids.waterBeaker));
                    }else if (material == Material.LAVA && ((Integer)iblockstate.getValue(BlockLiquid.LEVEL)).intValue() == 0){
                        playerIn.playSound(SoundEvents.ITEM_BUCKET_FILL_LAVA, 1.0F, 1.0F);
                        worldIn.setBlockState(blockpos, Blocks.AIR.getDefaultState(), 11);
                        playerIn.addStat(StatList.getObjectUseStats(this));
                        return new ActionResult(EnumActionResult.SUCCESS, this.fillBucket(itemStackIn, playerIn, ModFluids.lavaBeaker));
                    }else{
                        return new ActionResult(EnumActionResult.FAIL, itemStackIn);
                    }
                }
                ...
                if (!playerIn.canPlayerEdit(blockpos1, raytraceresult.sideHit, itemStackIn)){
                    ...
                    playerIn.addStat(StatList.getObjectUseStats(this));
                    return !playerIn.capabilities.isCreativeMode ? new ActionResult(EnumActionResult.SUCCESS, new ItemStack(ModFluids.beaker)) : new ActionResult(EnumActionResult.SUCCESS, itemStackIn);
                }else{
                    return new ActionResult(EnumActionResult.FAIL, itemStackIn);
                }
            }
