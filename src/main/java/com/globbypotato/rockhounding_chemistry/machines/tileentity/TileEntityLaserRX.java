		...
		if(!worldObj.isRemote){
			...
			if(isUnpoweredRx(state)){
				if(isReceivingLaser(state)){
					worldObj.setBlockState(pos, ModBlocks.laserRedstoneRx.getDefaultState().withProperty(LaserRX.VARIANT, EnumLaser.byMetadata(state.getBlock().getMetaFromState(state) + 1)));
				}
