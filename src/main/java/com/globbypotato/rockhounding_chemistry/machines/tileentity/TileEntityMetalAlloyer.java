...
public class TileEntityMetalAlloyer extends TileEntityLockable implements ITickable, ISidedInventory, IEnergyReceiver, IEnergyStorage {
	...
	public static String[] alloyNames = new String[]{"No Recipe", "CuBe", "ScAl", "BAM", "YAG", "Cupronickel", "Nimonic", "Hastelloy", "Nichrome", "Mischmetal", "Rose Gold", "Green Gold", "White Gold", "Shibuichi", "Tombak", "Pewter", "Corten Steel"};
	...
	private String[] getAlloy(int countRecipes) {
		...
		}else if(countRecipes == 16){ return cortenRecipe;}
		return null;
	}
	...
	private int[] getQuantity(int countRecipes) {
		...
		}else if(countRecipes == 16){ return cortenQuantity;}
		return null;
	}
	...
	private boolean canAlloy(String[] recipe, int[] quantity) {
		return  countRecipes() > 0 &&
			...
			|| (slots[SLOT_OUTPUT] != null && slots[SLOT_TEMPLATE] != null && slots[SLOT_OUTPUT].isItemEqual(slots[SLOT_TEMPLATE]) 
			&& slots[SLOT_OUTPUT].stackSize <= slots[SLOT_OUTPUT].getMaxStackSize() - 8)) 
