...
public class UIStirredTankController extends GuiBase {

	public static ResourceLocation TEXTURE_REF = new ResourceLocation(Reference.MODID + ":textures/gui/guistirredtankcontroller.png");
	public static ResourceLocation TEXTURE_JEI = new ResourceLocation(Reference.MODID + ":textures/gui/jei/guistirredtankcontrollerjei.png");

    private final TEStirredTankController tile;

    public UIStirredTankController(InventoryPlayer playerInv, TEStirredTankController tile){
    	super(new COStirredTankController(playerInv,tile), ModUtils.HEIGHT);
        GuiBase.TEXTURE = TEXTURE_REF;
        this.tile = tile;
		this.containerName = "container." + TEStirredTankController.getName();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
	   ...
	   if(GuiUtils.hoveringArea(30, 60, 16, 31, mouseX, mouseY, x, y)){
		   ...
		   if(this.tile.hasSolventTank()){
			   if(this.tile.getSolventTank().hasSolventFluid()){
				   solv = TextFormatting.WHITE + this.tile.getSolventTank().getSolventFluid().getLocalizedName() + ": "+ TextFormatting.AQUA + this.tile.getCalculatedSolvent() + " mB";
			   }
		   }
	   ...
	   if(GuiUtils.hoveringArea(130, 60, 16, 31, mouseX, mouseY, x, y)){
		   ...
		   if(this.tile.hasReagentTank()){
			   if(this.tile.getReagentTank().hasSolventFluid()){
				   solv = TextFormatting.WHITE + this.tile.getReagentTank().getSolventFluid().getLocalizedName() + ": "+ TextFormatting.AQUA + this.tile.getCalculatedReagent() + " mB";
			   }
		   }
		   multistring = new String[]{commDescr, solv};
		   tooltip = GuiUtils.drawMultiLabel(multistring, mouseX, mouseY);
