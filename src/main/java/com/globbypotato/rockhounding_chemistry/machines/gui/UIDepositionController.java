...
public class UIDepositionController extends GuiBase {
    ...
    private final TEDepositionController tile;

    public UIDepositionController(InventoryPlayer playerInv, TEDepositionController tile){
    	super(new CODepositionController(playerInv,tile), ModUtils.HEIGHT);
        this.tile = tile;
        GuiBase.TEXTURE = TEXTURE_REF;
		this.containerName = "container." + TEDepositionController.getName();
    }
	   ...
	   if(GuiUtils.hoveringArea(25, 20, 18, 18, mouseX, mouseY, x, y)){
		...
		if(GuiUtils.hoveringArea(7, 45, 12, 67, mouseX, mouseY, x, y)){
			...
			if(this.tile.isValidPreset()){
				req = TextFormatting.DARK_GRAY + "Required: " + TextFormatting.WHITE + this.tile.getRecipeList(this.tile.getSelectedRecipe()).getTemperature() + "K";
			}
		...
		if(GuiUtils.hoveringArea(157, 45, 12, 67, mouseX, mouseY, x, y)){
			...
			if(this.tile.isValidPreset()){
				req = TextFormatting.DARK_GRAY + "Required: " + TextFormatting.WHITE + this.tile.getRecipeList(this.tile.getSelectedRecipe()).getPressure() + " uPa";
			}
