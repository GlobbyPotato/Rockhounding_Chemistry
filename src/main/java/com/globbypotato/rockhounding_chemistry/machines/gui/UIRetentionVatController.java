...
public class UIRetentionVatController extends GuiBase {

	public static ResourceLocation TEXTURE_REF = new ResourceLocation(Reference.MODID + ":textures/gui/guiretentionvat.png");
	public static ResourceLocation TEXTURE_JEI = new ResourceLocation(Reference.MODID + ":textures/gui/jei/guiretentionvatjei.png");

    private final TERetentionVatController tile;

    public UIRetentionVatController(InventoryPlayer playerInv, TERetentionVatController tile){
    	super(new CORetentionVatController(playerInv,tile), ModUtils.HEIGHT);
        GuiBase.TEXTURE = TEXTURE_REF;
        this.tile = tile;
		this.containerName = "container." + TERetentionVatController.getName();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
	   ...
	   //pulp slot
		if(GuiUtils.hoveringArea(80, 28, 16, 16, mouseX, mouseY, x, y)){
			if(this.tile.getPulp() != null){
				tooltip = GuiUtils.drawFluidTankInfo(this.tile.getTankPulp().inputTank, mouseX, mouseY);
				drawHoveringText(tooltip, mouseX, mouseY, this.fontRenderer);
			}else{
			   tooltip = GuiUtils.drawLabel("Empty. No pulp to process", mouseX, mouseY);
			   drawHoveringText(tooltip, mouseX, mouseY, this.fontRenderer);
			}
       }
    ...
    public void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY){
        ...
        //show pulp
        if(this.tile.getPulp() != null){
			GuiUtils.renderFluidBar(this.tile.getPulp(), 1000, 1000, i + 80, j + 28, 16, 16);
        }
    }
