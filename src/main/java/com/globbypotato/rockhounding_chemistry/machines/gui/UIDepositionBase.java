...
public class UIDepositionBase extends GuiBase {

	public static ResourceLocation TEXTURE_REF = new ResourceLocation(Reference.MODID + ":textures/gui/guidepositionchamber.png");

    private final TEDepositionBase tile;

    public UIDepositionBase(InventoryPlayer playerInv, TEDepositionBase tile){
    	super(new CODepositionBase(playerInv,tile), ModUtils.HEIGHT);
        GuiBase.TEXTURE = TEXTURE_REF;
        this.tile = tile;
		this.containerName = "container." + TEDepositionBase.getName();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
	   ...
	   if(GuiUtils.hoveringArea(122, 49, 18, 18, mouseX, mouseY, x, y)){
		   if(this.tile.insulationSlot().isEmpty()){
			   tooltip = GuiUtils.drawLabel(BaseRecipes.misc_items(1, EnumMiscItems.DEPOSITION_INSULATION).getDisplayName(), mouseX, mouseY);
			   drawHoveringText(tooltip, mouseX, mouseY, this.fontRenderer);
		   }
	   }
	   ...
	   if(GuiUtils.hoveringArea(36, 49, 18, 18, mouseX, mouseY, x, y)){
		   if(this.tile.casingSlot().isEmpty()){
			   tooltip = GuiUtils.drawLabel(BaseRecipes.misc_items(1, EnumMiscItems.DEPOSITION_CASING).getDisplayName(), mouseX, mouseY);
			   drawHoveringText(tooltip, mouseX, mouseY, this.fontRenderer);
		   }
	   }
