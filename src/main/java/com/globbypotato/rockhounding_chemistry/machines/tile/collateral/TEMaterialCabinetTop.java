						...
						for(int x = 0; x < getCollector().MATERIAL_LIST.size(); x++){
							if(inputDict.matches(getCollector().MATERIAL_LIST.get(x).getOredict())){
								...
								if(storedAmount <= (ModConfig.extractorCap - ModConfig.extractorFactor)){
									...
									MaterialCabinetRecipe currentMaterial = new MaterialCabinetRecipe(getCollector().MATERIAL_LIST.get(x).getSymbol(), getCollector().MATERIAL_LIST.get(x).getOredict(), getCollector().MATERIAL_LIST.get(x).getName(), storedAmount, getCollector().MATERIAL_LIST.get(x).getExtraction());
