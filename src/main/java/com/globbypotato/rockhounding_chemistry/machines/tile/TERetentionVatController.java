...
public class TERetentionVatController extends TileEntityInv implements IInternalServer {
	...
	public TERetentionVatController() {
	...
	public static String getName(){
		return "retention_vat_controller";
	}
	...
	private void doPreset() {
		...
		if(hasVessel()){
			if(getVessel().getFilter() != steam()){
				getVessel().filter = steam();
			}
		}
...
//vessel1
	public TileVessel getVessel(){
		TileVessel vessel = TileStructure.getHolder(this.world, this.pos.offset(getFacing()).offset(isFacingAt(90), 2), isFacingAt(270));
		return vessel != null ? vessel : null;
	}

	public boolean hasVessel(){
		return getVessel() != null && hasCentrifuges();
	}

//centrifuge
	public TECentrifuge getSteamCentrifuge(){
	...
	public boolean hasCentrifuges(){
		return getSteamCentrifuge() != null;
	}
...
// tank waste
	public TEBufferTank getTankWaste(){
		BlockPos tankPos = this.pos.offset(getFacing(), 3).offset(EnumFacing.UP);
...
//process check
	public boolean hasLeacherA(){
		return (hasVatA() 
			...
			&& (hasVessel() 
			&& this.input.canDrainFluid(getVessel().inputTank.getFluid(), steam(), calculatedSteam()));
	}
...
//server
	...
	private boolean isAssembled() {
		return isOutputEmpty() && hasTankWaste() && hasTankPulp() && (hasLeacherA() && hasLeacherB());
	}
							...
							if(isCorrectDevice(tag, deviceCode())){
								if(getRecipe(tag) < 16){
									if(getDone(tag) > 0){
										...
										if(hasFilterFluid(tag) && hasTankPulp()){
