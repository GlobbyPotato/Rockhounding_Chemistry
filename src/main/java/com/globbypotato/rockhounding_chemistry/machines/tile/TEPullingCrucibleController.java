...
public class TEPullingCrucibleController extends TileEntityInv {
	...
	public TEPullingCrucibleController() {
	...
	public static String getName(){
		return "pulling_crucible_controller";
	}
...
//centrifuge
	public TECentrifuge getCentrifuge(){
		BlockPos centrifugePos = this.pos.offset(EnumFacing.UP).offset(isFacingAt(270));
		TECentrifuge centrifuge = TileStructure.getCentrifuge(this.world, centrifugePos, isFacingAt(90));
...
//charger
	public Block getCharger(){
		BlockPos chargerPos= this.pos.offset(EnumFacing.UP, 3);
...
//unloader
	public TEUnloader getUnloader(){
		BlockPos unloaderPos = this.pos.offset(isFacingAt(90), 1);
...
//input vessel
	public TileVessel getInTank(){
		BlockPos vesselpos = this.pos.offset(EnumFacing.UP).offset(isFacingAt(270), 2);
		TileVessel vessel = TileStructure.getHolder(this.world, vesselpos, isFacingAt(90));
...
//tower cap
	public Block crucibleCap(){
		Block separator = TileStructure.getStructure(this.world, this.pos.offset(EnumFacing.UP, 2), EnumMiscBlocksA.PULLING_CRUCIBLE_CAP.ordinal());
		return separator != null ? separator : null;
	}

	public boolean hasCap() {
		return crucibleCap() != null;
	}

//pressurizer
	public TEAuxiliaryEngine getPressurizer(){
		TEAuxiliaryEngine pressurizer = TileStructure.getPressurizer(this.world, this.pos.offset(isFacingAt(90), 2), isFacingAt(270));
...
//dopant
	...
	private boolean isAssembled() {
		return hasCentrifuge() && hasUnloader() && hasPressurizer() && hasCap() && hasCharger();
	}
		...
		return isActive()
			...
			&& canOutput();
				...
				private boolean canOutput() {
					return hasUnloader()
						&& ((MachineStackHandler) getUnloader().getOutput()).canSetOrStack(getUnloader().unloaderSlot(), getDummyRecipe().getOutput());
				}
	...
	private void process() {
		if(getDummyRecipe() != null && getDummyRecipe() == getCurrentRecipe()){
			...
			if(hasUnloader()) {
				((MachineStackHandler) getUnloader().getOutput()).setOrStack(OUTPUT_SLOT, output());
			}
