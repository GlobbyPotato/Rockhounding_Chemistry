...
public class TEElementsCabinetBase extends TileEntityInv {
	...
	public ArrayList<ElementsCabinetRecipe> MATERIAL_LIST = new ArrayList<ElementsCabinetRecipe>();
	public ArrayList<ElementsCabinetRecipe> TEMP_MATERIAL_LIST = new ArrayList<ElementsCabinetRecipe>();
	public ArrayList<ElementsCabinetRecipe> PAGED_MATERIAL_LIST = new ArrayList<ElementsCabinetRecipe>();
	...
	public void readFromNBT(NBTTagCompound compound){
		...
		if(recipes.tagCount() > 0) {
			this.MATERIAL_LIST = new ArrayList<ElementsCabinetRecipe>();
	        for (int i = 0; i < recipes.tagCount(); ++i){
				...
				this.MATERIAL_LIST.add(j, new ElementsCabinetRecipe(symbol, oredict, name, amount, extraction));
	        }
			this.reloadCabinet();
	...
	public class SymbolComparator implements Comparator<ElementsCabinetRecipe>{
	    public int compare(ElementsCabinetRecipe o1, ElementsCabinetRecipe o2){
	       return o1.getSymbol().compareToIgnoreCase(o2.getSymbol());
	   }
	}
	public class NameComparator implements Comparator<ElementsCabinetRecipe>{
	    public int compare(ElementsCabinetRecipe o1, ElementsCabinetRecipe o2){
			...
			if(slot < this.PAGED_MATERIAL_LIST.size()) {
				for(int x = 0; x < this.MATERIAL_LIST.size(); x++) {
					if(!this.PAGED_MATERIAL_LIST.get(slot).getSymbol().isEmpty() && this.PAGED_MATERIAL_LIST.get(slot).getSymbol().matches(this.MATERIAL_LIST.get(x).getSymbol())) {
						...
						status = !status;
						this.MATERIAL_LIST.set(x, new ElementsCabinetRecipe(this.MATERIAL_LIST.get(x).getSymbol(), this.MATERIAL_LIST.get(x).getOredict(), this.MATERIAL_LIST.get(x).getName(), this.MATERIAL_LIST.get(x).getAmount(), status));
					}
				}
			}
		}
	...
	public void initializaCabinet() {
		if(MATERIAL_LIST.isEmpty()) {
			for(ElementsCabinetRecipe recipe : recipeList()) {
	...
	public void reloadCabinet() {
		if(MATERIAL_LIST.size() < recipeList().size()) {
			ElementsCabinetRecipe missingRecipe = null;
			for(int y = 0; y < recipeList().size(); y++) {
				...
				missingRecipe = new ElementsCabinetRecipe(checkSymbol, checkDict, checkName, checkAmount, checkExtract);
...
//balance
	public TEExtractorBalance getBalance(){
		TEExtractorBalance balance = TileStructure.getBalance(this.world, this.pos, isFacingAt(270));
