...
public class GaslineHalt extends PipelineBase {
	public GaslineHalt(String name) {
    ...
    public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos, BlockPos sidePos, EnumFacing facing){
        ...
        return block instanceof GaslineDuct || block instanceof GaslineHalt ? true : false;
    }
	...
	protected static final void onRedstoneDeactivated (World worldIn, BlockPos pos, IBlockState state) {
		if (state.getValue(ISREDSTONEPOWERED).booleanValue())
			worldIn.setBlockState(pos, ModBlocks.GASLINE_DUCT.getDefaultState(), 2);
	}
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
		return Item.getItemFromBlock(ModBlocks.GASLINE_DUCT);
	}
